...
export const populateBadgeTracks = async (protocol: string) => {
  ...
  const badgeDefinitionsWithTracks = badgeDefinitions.map(
    (badgeDefinition) => ({
      ...badgeDefinition,
      // track: {
      //   id: removeRomanNumerals(badgeDefinition.id),
      //   protocol: {
      //     id: protocol,
      //   },
      // },
  ...
  BadgeDefinition.query().upsertGraph(badgeDefinitionsWithTracks);
